...
export function ProductCard({ product: p }: { product: Product }) {
  ...
  const available = p.variants.filter((v) => v.stock > v.reserved);
  return (
    <article className="product-card">
      <div className="product-photo">
        ...
        <button
          ...
          aria-label={`${favorites.includes(p.id) ? "Quitar de" : "Añadir a"} favoritos: ${p.name}`}
      ...
      <div className="product-meta">
        ...
        <p className="price">{cop(p.price)}</p>
        <div className="meta-line">
          {[...new Set(p.variants.map((v) => v.hex))].map((h) => (
            <span key={h} className="dot" style={{ background: h }} />
          ))}
          {available.length
            ? `${new Set(available.map((v) => v.size)).size} tallas disponibles`
            : "Sin disponibilidad"}
        </div>
      </div>
    </article>
    ...
    <>
      <section className="hero">
        <div className="hero-copy">
          <div className="eyebrow">Key & Naty · Un pequeño universo</div>
          <h1>
            {config.hero.title === "Pequeñas prendas. Grandes historias." ? (
              <>
                Pequeñas prendas.
                <br />
        ...
        <div className="hero-photo">
          <Image
            ...
            alt="Niños con prendas marfil y salvia; fotografía ilustrativa generada con IA"
            fill
            priority
            sizes="(max-width:760px) 100vw, 55vw"
          />
          <span className="photo-note">Imagen ilustrativa</span>
        </div>
      </section>
      <div className="container">
        <nav className="categories" aria-label="Categorías">
          {[
            ["Bebé", "6–24 meses", "overalls.webp"],
            ["Niña", "2–6 años", "dress.webp"],
            ["Niño", "2–6 años", "shirt.webp"],
          ].map(([n, a, i]) => (
            <Link
              key={n}
              className="category"
              href={"/catalogo?categoria=" + encodeURIComponent(n)}
            >
              <Image src={"/assets/" + i} alt="" width={70} height={78} />
              <div>
                <h3>{n}</h3>
                <span>{a} · Orientativo</span>
              </div>
              <ChevronRight />
            </Link>
          ))}
        ...
        <section className="section">
          <div className="section-head">
            <div>
              <div className="eyebrow">Para descubrir</div>
              <h2>La selección Key & Naty</h2>
            </div>
          ...
          {products.length ? (
            ...
            <p>
              La colección estará disponible cuando se publique el catálogo.
            </p>
          )}
        </section>
        {costumes.length > 0 && (
        ...
        <section className="story">
          ...
          <div>
            <p>
              Una primera salida, una tarde de juegos, un abrazo antes de
              dormir. Nuestra propuesta nace de esos momentos cotidianos y de
              las prendas que los acompañan.
            </p>
            <Link className="text-link" href="/nosotros">
              Conoce la propuesta <ArrowRight />
            </Link>
          </div>
        </section>
        <section className="service-row">
          <div>
            ...
            <p>
              Encuentra su talla
              <br />
              <small>Medidas específicas por prenda</small>
            </p>
          </div>
          <div>
            <Package />
            <p>
              ...
              <br />
              <small>Según el destino seleccionado</small>
            </p>
          </div>
          <div>
            <ShoppingBag />
            <p>
              Compra a tu manera
              <br />
              <small>Sin necesidad de crear una cuenta</small>
            </p>
          </div>
        </section>
      </div>
  ...
  const change = (key: string, value: string) => {
    ...
    const q = new URLSearchParams(
      Object.entries(next).filter(
        ([, v]) => v && v !== "Todo" && v !== "novedades",
      ),
    );
    window.history.replaceState(
      null,
      "",
      `${window.location.pathname}${q.size ? "?" + q : ""}`,
    );
  };
  const clear = () => {
    setFilters({
  ...
  const products = collectionProducts
    .filter(
      (p) =>
        ...
        (!filters.color || p.variants.some((v) => v.color === filters.color)) &&
        (!filters.edad || p.age === filters.edad) &&
        (!filters.ocasion || p.occasion === filters.ocasion) &&
      ...
      <div className="category-tabs" aria-label="Categoría">
        {(collection
          ...
          <button
            ...
            aria-pressed={filters.categoria === c}
            onClick={() => change("categoria", c)}
          >
            {c}
          </button>
        ))}
      ...
      <div className="catalog-layout">
        ...
        >
          <details className="filter-group" open>
            <summary>Talla disponible</summary>
            <div className="filter-sizes">
              {[
                ...
                <button
                  key={s}
                  className="size-btn"
                  aria-pressed={filters.talla === s}
                  onClick={() => change("talla", filters.talla === s ? "" : s)}
                >
                  {s}
                </button>
              ))}
          ...
          <details className="filter-group" open>
            ...
            >
              ...
              {[80000, 90000, 100000, 150000].map((n) => (
        ...
        <section>
          ...
          <div className="catalog-toolbar">
            ...
            <small role="status">{products.length} prendas</small>
            <select
          ...
          <div className="chips">
            {Object.entries(filters)
              .filter(([k, v]) => v && v !== "Todo" && k !== "orden")
              .map(([k, v]) => (
                <button
                  className="chip"
                  key={k}
                  onClick={() => change(k, k === "categoria" ? "Todo" : "")}
                >
                  {v} ×
                </button>
              ))}
          ...
          {!products.length && (
            <div className="empty">
              ...
              <p>
                {favoritesOnly
                  ? "Guarda prendas con el corazón para volver a verlas."
                  : "Prueba otra talla o explora la colección completa."}
              </p>
          ...
          <p className="catalog-bottom">
            La disponibilidad se verifica de nuevo al confirmar la compra.
          </p>
        </section>
      </div>
    </div>
